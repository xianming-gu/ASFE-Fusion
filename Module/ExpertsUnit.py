...
class Experts(nn.Module):
    ...
    def forward(
        ...
        shape, num_experts = x.shape, self.num_experts  # 4 4 256 224
...
class SpaFreExpFusion(Module):
    def __init__(
        ...
        super().__init__()
        assert exists(seq_len) or exists(num_slots), 'either seq_len, or num_slots must be passed into SoftMoE'
    ...
    def forward(self, x, mask=None, add_noise=False, noise_mult=1.):
        ...
        if is_image:
            x = rearrange(x, 'b d h w -> b h w d')  # 4 224 256 256 -> 4 256 256 224
            x, ps = pack([x], 'b * d')  # x:4 (256*256) 224 ps: 256*256
        ...
        logits = einsum('b n d, e s d -> b n e s', x, slot_embeds)  # 4, 65536, 4, 256
        ...
        dispatch_weights = logits.softmax(dim=1)  # 4, 65536, 4, 256

        combine_weights = rearrange(logits, 'b n e s -> b n (e s)')  # 4, 65536, 1024
        ...
        slots = einsum('b n d, b n e s -> b e s d', x, dispatch_weights)  # 4 4 256 224
...
if __name__ == '__main__':
    device = 'cuda:5'
    img1 = torch.randn(1, 48, 256, 256).to(device)
    img2 = torch.randn(1, 1, 256, 256).to(device)
    model = SpaFreExpFusion(dim=48).to(device)
    result = model(img1)
    print(result.shape)
